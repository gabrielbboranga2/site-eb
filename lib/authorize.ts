...
export async function authorizedFor(request:Request,channels:Channel[]){const user=await getSessionUser<{exp:number;roleId?:string;rankNumber?:number;id:string}>(request);if(!user)return false;const main=(await getUserGroupMemberships(user.id)).find(g=>g.groupId===521106467);if(!main)return false;const permissions=await readPermissions();return channels.some(channel=>canView(main.roleId,channel,permissions,main.rankNumber))}
