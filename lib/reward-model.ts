...
export interface RewardCode {id:string;title:string;kind:RewardKind;instructions:string;groupId:number|null;targetRoleId:string|null;targetName:string|null;maxUses:number;uses:number;expiresAt:string|null;active:boolean;createdAt:string}
...
export function normalizeRewardCode(value:unknown){if(typeof value!=='string')throw Error('Informe o código recebido.');const code=value.trim().toUpperCase();if(!/^MIG-[A-F0-9]{8}(?:-[A-F0-9]{8}){3}$/.test(code))throw Error('Código inválido. Confira os caracteres.');return code}
export function validateRewardInput(body:Record<string,unknown>){
 ...
 if(kind==='rank'&&(!Number.isSafeInteger(groupId)||(groupId??0)<=0||!/^\d+$/.test(targetRoleId!)))throw Error('Selecione uma comunidade e uma patente.');
 return {title,instructions,kind:kind as RewardKind,maxUses,expiresAt,groupId,targetRoleId};
}
