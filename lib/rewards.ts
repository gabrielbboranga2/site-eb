...
type CodeRow={id:string;code_label?:string|null;title:string;kind:'rank'|'external';instructions:string;group_id:number|null;target_role_id:string|null;target_name:string|null;max_uses:number;uses:number;expires_at:Date|null;active:boolean;created_at:Date};
...
export async function ensureRewardSchema(){
 if(!ready)ready=(async()=>{
  ...
  await db()`ALTER TABLE manager_reward_codes ADD COLUMN IF NOT EXISTS code_label text`;
...
function mapCode(r:CodeRow,showCode=false):RewardCode{return{id:r.id,...(showCode&&r.code_label?{code:r.code_label}:{}),title:r.title,kind:r.kind,instructions:r.instructions,groupId:r.group_id?Number(r.group_id):null,targetRoleId:r.target_role_id,targetName:r.target_name,maxUses:r.max_uses,uses:r.uses,expiresAt:r.expires_at?new Date(r.expires_at).toISOString():null,active:r.active,createdAt:new Date(r.created_at).toISOString()}}
...
async function ensureDefaultCodes(){for(const code of ['MIG','BIEL','UPFREE'])await db()`INSERT INTO manager_reward_codes(id,code_hash,code_label,title,kind,instructions,group_id,target_role_id,target_name,max_uses,created_by) VALUES(${randomUUID()},${hash(code)},${code},${'UP extra · '+code},'rank','Avanço de uma patente no Exército, respeitando a hierarquia e a CDP.',521106467,'__NEXT__','1 UP extra',10,'system') ON CONFLICT(code_hash) DO NOTHING`}
export async function rewardOverview(actor:Actor,creator=false){await ensureRewardSchema();await ensureDefaultCodes();const claims=creator?await db()<ClaimRow[]>`SELECT c.*,r.id,r.code_id,r.user_id,r.username,r.status,r.note,r.created_at FROM manager_reward_claims r JOIN manager_reward_codes c ON c.id=r.code_id ORDER BY r.created_at DESC LIMIT 200`:await db()<ClaimRow[]>`SELECT c.*,r.id,r.code_id,r.user_id,r.username,r.status,r.note,r.created_at FROM manager_reward_claims r JOIN manager_reward_codes c ON c.id=r.code_id WHERE r.user_id=${actor.id} ORDER BY r.created_at DESC LIMIT 100`;const codes=creator?await db()<CodeRow[]>`SELECT * FROM manager_reward_codes ORDER BY created_at DESC LIMIT 200`:[];const profiles=await getRobloxIdentities(claims.map(r=>r.user_id));return{claims:claims.map(r=>({...mapClaim(r),avatar:profiles.get(r.user_id)?.avatar,username:profiles.get(r.user_id)?.username||r.username})),codes:codes.map(r=>mapCode(r,true))}}
export async function createReward(body:Record<string,unknown>,actor:Actor){
 ...
 if(input.kind==='rank'){const group=(await getLiveHierarchies()).find(g=>g.groupId===input.groupId);if(input.targetRoleId==='__NEXT__')targetName='1 UP extra';else{const role=group?.roles.find(r=>r.id===input.targetRoleId);if(!role||role.rank<=0||role.rank>=253)throw Error('Esta patente não pode ser entregue por código.');targetName=group!.sigla+' · '+role.name;}}
 await ensureRewardSchema();const code=input.code;
 const id=randomUUID();try{await db().begin(async sql=>{
 await sql`INSERT INTO manager_reward_codes(id,code_hash,code_label,title,kind,instructions,group_id,target_role_id,target_name,max_uses,expires_at,created_by) VALUES(${id},${hash(code)},${code},${input.title},${input.kind},${input.instructions},${input.groupId},${input.targetRoleId},${targetName},${input.maxUses},${input.expiresAt},${actor.id})`;
 await sql`INSERT INTO manager_activity(id,tipo,user_id,username,descricao,autor_id,autor_username) VALUES(${randomUUID()},'resgate',${actor.id},${actor.username},${'Nova recompensa configurada · '+(targetName||'Entrega manual')},${actor.id},${actor.username})`;
 });}catch(error){if((error as {code?:string}).code==='23505')throw Error('Este código já está em uso. Escolha outro.');throw error}return{code};
}
...
async function validateRecipient(r:CodeRow,actor:Actor){
 ...
 if(r.kind==='rank'){let targetRoleId=r.target_role_id!;if(targetRoleId==='__NEXT__'){const group=(await getLiveHierarchies()).find(g=>g.groupId===Number(r.group_id));const next=group?.roles.filter(role=>role.rank>membership.rankNumber&&role.rank<253).sort((a,b)=>a.rank-b.rank)[0];if(!next)throw Error('Você já está na patente máxima disponível para este código.');targetRoleId=next.id;}const change=await previewDirectRankChange(actor.id,Number(r.group_id),targetRoleId);if(change.target.rank<=change.current.rank)throw Error('Você já possui esta patente ou uma superior. O código não foi consumido.');if(Number(r.group_id)===521106467)await assertPromotionCdpAvailable(actor.id);return change;}return null;
}
export async function redeemReward(code:string,actor:Actor){
 ...
 await db().begin(async sql=>{
  ...
  await sql`INSERT INTO manager_activity(id,tipo,user_id,username,descricao,autor_id,autor_username) VALUES(${randomUUID()},'resgate',${actor.id},${actor.username},'Recompensa resgatada',${actor.id},${actor.username})`;
 });
 if(r.kind==='external')return{message:'Resgate registrado. Um Criador confirmará a entrega.',status:'pending'};
 try{
  const result=await applyDirectRankChange(actor.id,Number(r.group_id),change!.target.id,change!.current.id);
...
export async function manageReward(body:Record<string,unknown>,actor:Actor){
 ...
 if(body.action==='disable'){await db().begin(async sql=>{const rows=await sql`UPDATE manager_reward_codes SET active=false WHERE id=${id} AND active=true RETURNING id`;if(!rows.length)throw Error('Código já encerrado ou inexistente.');await sql`INSERT INTO manager_activity(id,tipo,user_id,username,descricao,autor_id,autor_username) VALUES(${randomUUID()},'resgate',${actor.id},${actor.username},'Recompensa encerrada por um Criador',${actor.id},${actor.username})`});return;}
 ...
 if(r.kind==='rank'){
  ...
  if(r.target_role_id==='__NEXT__')throw Error('O UP extra precisa ser conferido diretamente na patente atual e no histórico antes da confirmação.');
