...
export function TrainingRegistration({instructor,instructorRank=0,instructorRole='',membershipRanks=[],preview=false}:Props){
  ...
  const[verificationCode,setVerificationCode]=useState(preview?'MIG-DEMO00-0000':'');
  const[verificationState,setVerificationState]=useState<'idle'|'checking'|'valid'>('idle');
  const[verificationMeta,setVerificationMeta]=useState<{label:string;ruleId:string;range:string;expiresAt:string}|null>(null);
  ...
  const codeRequired=community==='EXÉRCITO';
  const canSubmit=participants.length>0&&!!training&&isEligible&&observation.trim().length>=10&&!!proof&&(!codeRequired||verificationState==='valid')&&status!=='sending';
  ...
  function changeCommunity(next:TrainingCommunity){
    ...
    setCommunity(next);setTraining(firstAllowed.id);setVerificationState('idle');setVerificationMeta(null);setError('');setStatus('editing');
  }
  function changeTraining(next:string){setTraining(next);setVerificationState('idle');setVerificationMeta(null);setError('')}
  async function verifyCode(){
    setError('');setVerificationState('checking');setVerificationMeta(null);
    if(preview){setTraining('normal');setVerificationMeta({label:'Treinamento Normal',ruleId:'normal',range:'Recruta até Soldado',expiresAt:new Date(Date.now()+21600000).toISOString()});setVerificationState('valid');return}
    try{
      const response=await fetch('/api/training-codes/verify',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:verificationCode})});
      const payload=await response.json();
      if(!response.ok)throw new Error(payload.message||'Código inválido.');
      setCommunity('EXÉRCITO');setTraining(payload.data.ruleId);setVerificationMeta(payload.data);setVerificationState('valid');
    }catch(caught){setVerificationState('idle');setError(caught instanceof Error?caught.message:'Não foi possível verificar o código.')}
  }
  ...
  async function submit(event:FormEvent){
    ...
    if(!canSubmit){setError(codeRequired&&verificationState!=='valid'?'Valide o código gerado dentro do servidor antes de registrar.':'Preencha o treinamento, participantes, relatório e anexe a foto obrigatória.');return}
    setStatus('sending');
    if(preview){
    ...
    try{
      ...
      body.set('observation',observation.trim());body.set('proof',proof as File);body.set('verificationCode',verificationCode);
  ...
  function reset(){
    setParticipants(preview?['gabribor-sola']:[]);setParticipantInput('');setObservation('');setProof(null);setProofUrl('');setVerificationCode(preview?'MIG-DEMO00-0000':'');setVerificationState('idle');setVerificationMeta(null);setReceipt('');setError('');setStatus('editing');
  }
  ...
  return<form className="training-workflow" onSubmit={submit}>
    ...
    <section className="panel instructor-overview"><div className="instructor-summary"><span className="instructor-seal">IN</span><div><span className="kicker">INSTRUTOR</span><h2>{instructor}</h2><p>{eligibleCount} de {options.length} treinamentos liberados em {community}</p></div><div className="instructor-metric"><strong>{eligibleCount}</strong><small>LIBERADOS</small></div></div><div className="training-rule-head"><div><span className="kicker">TREINOS QUE VOCÊ PODE CONDUZIR</span><h3>Elegibilidade por patente</h3></div><small>Cargo atual: {effectiveRole||`nível ${effectiveRank}`}</small></div><div className="training-rule-grid">{options.map(rule=>{const allowed=effectiveRank>=rule.minInstructorRank;return<button type="button" key={rule.id} className={`${training===rule.id?'selected ':''}${allowed?'allowed':'locked'}`} onClick={()=>allowed&&changeTraining(rule.id)} aria-disabled={!allowed}><span className="training-rule-code">{rule.code}</span><div><b>{rule.name}</b><p>{rule.result}</p><small>{allowed?'LIBERADO':`Exige ${rule.minInstructorRole}`}</small></div><em>{allowed?'✓':'🔒'}</em></button>})}</div></section>
    <div className="training-layout"><section className="panel training-form-panel">
      <div className="form-section"><div className="form-section-head"><span>01</span><div><b>Comunidade e instrução</b><small>Selecione onde o treinamento foi realizado.</small></div></div><div className="community-picker">{TRAINING_COMMUNITIES.map(item=><button type="button" key={item} className={community===item?'active':''} onClick={()=>changeCommunity(item)}>{item}</button>)}</div><label className="field-label" htmlFor="training-type">Tipo de treinamento</label><select id="training-type" value={training} onChange={event=>changeTraining(event.target.value)}>{options.map(rule=><option key={rule.id} value={rule.id} disabled={effectiveRank<rule.minInstructorRank}>{rule.name}{effectiveRank<rule.minInstructorRank?` — exige ${rule.minInstructorRole}`:''}</option>)}</select>{!isEligible&&<div className="training-lock-note">Seu cargo atual não pode conduzir este treinamento. Patente mínima: <b>{selectedRule.minInstructorRole}</b>.</div>}</div>
      {codeRequired&&<div className="form-section verification-section"><div className="form-section-head"><span>02</span><div><b>Código do servidor</b><small>Use o código exibido ao dono do servidor reservado.</small></div><em>USO ÚNICO</em></div><div className={`verification-box ${verificationState}`}><div className="verification-code-row"><input aria-label="Código de verificação do treinamento" value={verificationCode} maxLength={16} placeholder="MIG-XXXXXX-XXXX" onChange={event=>{setVerificationCode(event.target.value.toUpperCase());setVerificationState('idle');setVerificationMeta(null)}}/><button type="button" className="secondary" disabled={verificationState==='checking'||verificationCode.length<14} onClick={()=>void verifyCode()}>{verificationState==='checking'?'Verificando…':verificationState==='valid'?'Verificado':'Validar código'}</button></div>{verificationMeta?<div className="verification-result"><span>✓</span><div><b>{verificationMeta.label}</b><small>{verificationMeta.range} · vinculado à conta de {instructor}</small></div></div>:<p>O código confirma o instrutor, a modalidade e que o servidor foi aberto para treinamento.</p>}</div></div>}
      <div className="form-section"><div className="form-section-head"><span>{codeRequired?'03':'02'}</span><div><b>Militares participantes</b><small>Adicione de 1 a 15 usernames do Roblox.</small></div><em>{participants.length}/15</em></div><div className="participant-entry"><input aria-label="Username do participante" placeholder="Ex.: gabribor-sola" value={participantInput} onChange={event=>setParticipantInput(event.target.value)} onKeyDown={participantKeyDown}/><button type="button" className="secondary" onClick={addParticipants} disabled={!participantInput.trim()||participants.length>=15}>Adicionar</button></div>{participants.length>0&&<div className="participant-chips">{participants.map(name=><span key={name}>{name}<button type="button" aria-label={`Remover ${name}`} onClick={()=>setParticipants(current=>current.filter(item=>item!==name))}>×</button></span>)}</div>}</div>
      <div className="form-section"><div className="form-section-head"><span>{codeRequired?'04':'03'}</span><div><b>Relatório do instrutor</b><small>Registre o desempenho e qualquer ocorrência relevante.</small></div></div><textarea aria-label="Relatório do treinamento" rows={5} maxLength={1000} placeholder="Descreva como foi o treinamento, desempenho dos participantes e observações..." value={observation} onChange={event=>setObservation(event.target.value)}/><small className="char-count">{observation.length}/1000 · mínimo de 10 caracteres</small></div>
      <div className="form-section proof-section"><div className="form-section-head"><span>{codeRequired?'05':'04'}</span><div><b>Prova fotográfica obrigatória</b><small>Foto dos jogadores em formação de cunha com a bandeira do Brasil visível ao fundo.</small></div><em>OBRIGATÓRIO</em></div><label className={`proof-uploader ${proof?'has-file':''}`}><input type="file" accept="image/jpeg,image/png,image/webp" onChange={chooseProof}/>{proofUrl?<img src={proofUrl} alt="Prévia da prova do treinamento"/>:<span className="proof-icon">IMG</span>}<div><b>{proof?proof.name:'Selecionar foto da cunha'}</b><small>{proof?`${(proof.size/1024/1024).toFixed(2)} MB · pronta para envio`:'JPG, PNG ou WEBP · máximo 8 MB'}</small></div><strong>{proof?'Trocar':'Anexar'}</strong></label></div>
    ...
    </section><aside className="panel training-summary"><span className="kicker">CONFERÊNCIA</span><h2>Resumo do registro</h2><dl><div><dt>Instrutor</dt><dd>{instructor}</dd></div><div><dt>Comunidade</dt><dd>{community}</dd></div><div><dt>Treinamento</dt><dd>{selectedRule.name}</dd></div>{codeRequired&&<div><dt>Código do servidor</dt><dd className={verificationState==='valid'?'ready':''}>{verificationState==='valid'?'Validado':'Pendente'}</dd></div>}<div><dt>Patente mínima</dt><dd>{selectedRule.minInstructorRole}</dd></div><div><dt>Participantes</dt><dd>{summary}</dd></div><div><dt>Prova da cunha</dt><dd className={proof?'ready':''}>{proof?'Pronta':'Pendente'}</dd></div></dl><div className="discord-route"><span>DC</span><div><b>Destino automático</b><small>{preview?'Discord simulado no preview':'Canal de treinamentos do Discord'}</small></div></div><p className="proof-guidance"><b>A foto precisa comprovar o treino.</b> Confirme a formação em cunha, os jogadores visíveis e a bandeira brasileira ao fundo antes de enviar.</p></aside></div>
