'use client';
import {RankChangeWorkflow} from './RankChangeWorkflow';
...
import {CHANNELS,DEFAULT_PERMISSIONS,canView,type Channel,type Permissions} from '@/lib/access';
...
export const demoMembers:RosterMember[]=[{userId:'1001',username:'LucasMIG',rankName:'[CB] Cabo',division:'BPE',roleId:'808664027',rankNumber:3},{userId:'1002',username:'AnaComandos',rankName:'[SLD] Soldado',division:'BAC',roleId:'807380036',rankNumber:2},{userId:'1003',username:'PedroTatico',rankName:'[3° SGT] Terceiro Sargento',division:'BFE',roleId:'808768016',rankNumber:4},{userId:'1004',username:'MarinaIntel',rankName:'[CAP] Capitão',division:'CIE',roleId:'808906015',rankNumber:12}];
...
export default function Manager({user,preview=false}:{user:SessionUser;preview?:boolean}){
 ...
 const creator=preview?demoRole==='creator':user.isCreator===true;
 const visible=preview?CHANNELS.filter(c=>canView(creator?'808700015':'807380036',c,permissions,creator?255:2)):allowed;
 const load=async()=>{
  ...
  try{
   ...
   setAllowed(access.allowed);if(access.permissions)setPermissions(access.permissions);setCanSave(access.canSave);setRoles(access.roles||[]);
   ...
   if(['Início','Estatísticas','CDP','Logs globais','Histórico de patentes','Ranking','Meu perfil'].includes(active)){
    ...
    if(current!==generation.current)return;
    setData({...stats,atividadesRecentes:audit.logs,cdpMembers:audit.cdpMembers,auditAvailable:audit.auditAvailable,emCdp:audit.cdpMembers.length,acoesRegistradas:audit.acoesRegistradas,treinosMes:audit.treinosMes});
   }
  }catch(e){if(current===generation.current){setAllowed([]);setData(null);setError(e instanceof Error?e.message:'Falha na consulta.')}}
 ...
 useEffect(()=>{if(preview)return;fetch('/api/announcements',{cache:'no-store'}).then(r=>r.json()).then(d=>setAnnouncements(d.announcements||[])).catch(()=>{})},[preview]);
 useEffect(()=>{if(!menu)return;const handler=(e:KeyboardEvent)=>{if(e.key==='Escape')setMenu(false)};document.addEventListener('keydown',handler);return()=>document.removeEventListener('keydown',handler)},[menu]);
 function navigate(page:string){if(page!==active){generation.current++;setData(null);setLoading(true)}setActive(page);setMenu(false);window.scrollTo({top:0,behavior:'instant'})}
 ...
 {active==='Início'&&<Overview user={user} data={data} navigate={navigate} visible={visible}/>}
 {active==='Meu perfil'&&<><section className="m-panel profile-panel"><Avatar name={user.username} src={user.avatar}/><h2>{user.username}</h2><span className="m-tag">{user.rank||'Membro'}</span><div className="profile-facts"><div><small>Divisão</small><b>{user.division||'EXÉRCITO'}</b></div><div><small>ID Roblox</small><b>{user.id}</b></div></div>{!preview&&<a className="m-button" href={`https://www.roblox.com/users/${user.id}/profile`} target="_blank" rel="noreferrer">Ver no Roblox ↗</a>}</section><EmblemGallery preview={preview}/></>}
 {active==='Estatísticas'&&<Statistics data={data}/>}{active==='Militares'&&<Members preview={preview}/>}{active==='Hierarquia'&&<Hierarchy preview={preview}/>}{active==='CDP'&&<Cdp data={data} user={user}/>}{['Logs globais','Histórico de patentes'].includes(active)&&<Logs key={active} logs={data?.atividadesRecentes||[]} history={active==='Histórico de patentes'} available={data?.auditAvailable}/>}{active==='Ranking'&&<Ranking logs={data?.atividadesRecentes||[]}/>}{['Entregar patente','Promoção em divisão','Rebaixamentos'].includes(active)&&<RankChangeWorkflow key={active} mode={active==='Rebaixamentos'?'demotion':'promotion'} channel={active} preview={preview}/>}{active==='Treinamentos'&&<TrainingRegistration instructor={user.username} instructorRank={preview?255:user.rankNumber||0} instructorRole={user.rank} membershipRanks={user.divisions?.map(d=>({sigla:d.name,rank:d.rankNumber||0,role:d.role}))} preview={preview}/>}
 {active==='Configurações'&&<section className="m-panel prose"><h2>Aparência</h2><p>Escolha o tema. A preferência vale apenas para este navegador.</p><div className="tabs">{[['dark','Escuro'],['light','Claro'],['system','Sistema']].map(([v,label])=><button aria-pressed={theme===v} className={theme===v?'active':''} key={v} onClick={()=>changeTheme(v)}>{label}</button>)}</div><h2>Minha conta</h2><p>Se sua patente ou divisão mudar, saia e entre novamente para atualizar sua sessão.</p>{creator&&<button className="m-button primary" onClick={()=>navigate('Gestão do criador')}>Abrir gestão do criador</button>}</section>}
...
function Overview({user,data,navigate,visible}:{user:SessionUser;data:ManagerData|null;navigate:(s:string)=>void;visible:readonly string[]}){
 ...
 return <>
  <section className="welcome-panel command-briefing">
   ...
   <div className="command-pass" aria-label="Identificação militar atual">
    ...
    <div className="command-pass-foot"><span>{user.division||'EXÉRCITO'}</span><span>Roblox #{user.id}</span></div>
   </div>
  </section>
