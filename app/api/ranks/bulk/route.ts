...
export async function POST(request:Request){
  ...
  try{
    ...
    for(const userId of result.succeededUserIds)await recordRankActivity({eventId:crypto.randomUUID(),userId,username:profiles.get(userId)?.username||'Perfil indisponível',groupId,community:result.preview.community,promoted:direction==='promotion',description:result.preview.current.name+' → '+result.preview.target.name,actorId:session.id,actorUsername:session.username,trainingKey:training.key,trainingLabel:training.label});
