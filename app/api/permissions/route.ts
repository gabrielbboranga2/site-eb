...
export async function GET(request:Request){const user=await getSessionUser<{exp:number;id:string;roleId?:string;rankNumber?:number}>(request);if(!user)return NextResponse.json({error:'Não autorizado.'},{status:401});try{const main=(await getUserGroupMemberships(user.id)).find(g=>g.groupId===521106467);const permissions=await readPermissions();const groups=await getLiveHierarchies().catch(()=>getHierarchySnapshot());const roles=groups.find(g=>g.groupId===521106467)?.roles||[];return NextResponse.json({creator:isCreatorRole(main?.roleId),roles,allowed:CHANNELS.filter(c=>canView(main?.roleId,c,permissions,main?.rankNumber)),permissions:isCreatorRole(main?.roleId)?permissions:undefined,canSave:isCreatorRole(main?.roleId)&&persistentSettingsAvailable()},{headers:{'cache-control':'no-store'}})}catch{return NextResponse.json({error:'Não foi possível verificar as permissões.'},{status:503})}}
